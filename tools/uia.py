...
def invoke_element_by_name(hwnd, name_substr: str, min_y: int = 0):
    """在窗口控件树里按名找控件并直接编程触发(InvokePattern)。
    比 鼠标点坐标 可靠——不依赖坐标换算/DPI/窗口位置。
    hwnd: 目标窗口句柄; name_substr: 控件名子串; min_y: 只考虑 y>=min_y 的控件(0=不限)。
    返回 True 表示成功触发; 找不到/不支持 Invoke 返回 False。"""
    ...
    try:
        ...
        def _walk(el, depth):
            if hit[0] is not None or depth > _WALK_MAX_DEPTH:
                return
            try:
            ...
            try:
                ...
                return
            while child is not None and hit[0] is None:
                _walk(child, depth + 1)
                try:
        ...
        if hit[0] is None:
            return False
        # InvokePattern(PID=10000) 编程触发; 不支持时抛异常返回 False
        invoke = hit[0].GetCurrentPattern(10000)
        pat = invoke.QueryInterface(UIA.IUIAutomationInvokePattern)
        pat.Invoke()
        return True
    except Exception as exc:
        print("[uia] Invoke 触发失败（%s）" % exc)
        return False
    finally:
        try:
            comtypes.CoUninitialize()
...
def set_edit_value(hwnd, name_substr: str, value: str) -> bool:
    """找编辑框(控件名含 name_substr)，用 ValuePattern 设置文本。"""
    comtypes.CoInitialize()
    try:
        UIA = _import_uia_module()
        core = comtypes.client.CreateObject(
        ...
        def _walk(el, depth):
            ...
            try:
                ...
                if ct == 50004 and name_substr.lower() in name.lower():  # Edit
                    hit[0] = el
                    return
            except Exception:
                return
        ...
        if hit[0] is None:
            return False
        val = hit[0].GetCurrentPattern(10002)  # ValuePattern
        vp = val.QueryInterface(UIA.IUIAutomationValuePattern)
        vp.SetValue(value)
        return True
    except Exception as exc:
        print("[uia] SetValue 失败（%s）" % exc)
        return False
    finally:
        try:
            comtypes.CoUninitialize()
...
def find_element_by_type_and_name(hwnd, control_type: int, name_substr: str, max_y: int = 0) -> bool:
    """按类型+名字找控件并 Invoke；max_y>0 时只考虑 y<=max_y（搜索结果区在上半屏）"""
    comtypes.CoInitialize()
    try:
        UIA = _import_uia_module()
        core = comtypes.client.CreateObject(
        ...
        def _walk(el, depth):
            ...
            try:
                name = (el.CurrentName or "").strip()
                ct = el.CurrentControlType
                if ct == control_type and name_substr.lower() in name.lower():
                    r = el.CurrentBoundingRectangle
                    if max_y == 0 or r.top <= max_y:
                        hit[0] = el
                        return
            except Exception:
                return
        ...
        print("[uia] Find+Invoke 失败（%s）" % exc)
        return False
    finally:
        try:
            comtypes.CoUninitialize()
